...
=== Terminology
The raw argv[] values passed to the program are "*arguments*".

Following POSIX conventions, this document uses the terms "*options*" to refer
to named arguments, sometimes known as switches or flags. And "*operands*" for
...
If you want a little more control over error output you can use cli.parse() and
then inspect the results with cli.exitCode(), cli.errMsg(), and
cli.errDetail().
...
context, you can make an error handler that doesn't have the results passed to
it directly.
...
=== How Parsing Works
The parsing process goes through 3 phases shown below and allows registered
...
There can be any number of before actions, they are called in order of priority
(largest first) with the order added as the tie breaker.
...
An option can have any number of check actions and they are called in order
of priority (largest first) with the order added as the tie breaker.
...
referenced by the command line!) registered option, be called. These calls are
in order of priority and appearance. Like other option actions they are called
with the three parameters that are references to cli, opt, and the value string
...
void apple(Dim::Cli & cli) {
    cout << "It's a " << *color << " apple" << (*yell ? "!!!" : ".");
}

void orange(Dim::Cli & cli) {
...
[source, C++, test alt, test repl 6 1 10 1]
////
    cout << "It's a " << color << " apple" << (yell ? "!!!" : ".");
    ...
    cout << "It's an orange" << (yell ? "!!!" : ".");
...
[source, C++, test alt, test repl 18 2]
----
    if (!cli.parse(argc, argv))
        return cli.printError(cerr);
...
static void myCmd(Dim::Cli &) {
    cout << "Options: " << s_opts.option1 << ", " << s_opts.option2
        << ", and " << s_opts.option3 << endl;
}
----
...
==== Exec Actions
If you have some code that you want to run for all commands that makes sense to
separate from the code making the cli.exec() call, you can use cli.execBefore()
and cli.execAfter() to register exec actions to it.

Let's add this source file to the "In Seperate File" example above to time how
long commands take to finish.
[source, C++, test file timer]
----
// timer.cpp
#include <chrono>
...
static void startTimer(Dim::Cli & cli) {
    s_info.startTime = system_clock::now();
}

static void elapsedTime(Dim::Cli & cli) {
    // Don't print the elapsed time for help text or usage errors.
    if (!cli.parseAborted()) {
        auto elapsed = system_clock::now() - s_info.startTime;
        auto mins = duration_cast<minutes>(elapsed);
        cout << "Elapsed time: " << mins.count() << " minutes"
            << endl;
    }
}
...
TimerInfo::TimerInfo() {
    Dim::Cli cli;
    cli.beforeExec(startTimer)
        .afterExec(elapsedTime);
}
----

Now we get an extra line with the time!
[source, shell session]
----
$ a.out my
Options: 1, two, and three
...
changed via cli.iostreams(). Although if you're going that far it might be time
to make your own --help option.
...
=== Going Your Own Way
If the generated help doesn't work for you, you can override the built-in help
with your own.
...
Within your help printer you can use the existing functions to do some or all
of the work:
