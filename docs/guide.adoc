...
[source, shell session, test noscript]
----
$ export AOUT_OPTS=
$ a.out c d
Words: 'c' 'd'
...
int main(int argc, char * argv[]) {
    ...
    auto & args = cli.optVec<string>("[program and arguments]")
...
----
$ a.out --help
Usage: a.out [OPTIONS] [<program and arguments>...]
  program and arguments  Complicated thing to do.
...
| Usage
| cli.opt()
| Generated text listing the defined operands.
