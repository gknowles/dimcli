...
** copyable
** assignable from string, constructible from string, has an istream extraction
...
* Optional positionals are populated only if there are enough other
...
* If there is a required positional with unlimited arity it will prevent any
  ...
  arguments before they get a turn.
