...
| Executes the action of the matched command, optionally parsing first, 
returning true if it was attempted, false if usage error or otherwise short 
circuited. On failure it's expected to have set exitCode, errMsg, and 
optionally errDetail via fail().
