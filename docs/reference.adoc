...
| Add action to run after environment variable and response file expansion but
before any individual arguments are parsed.

| cli.beforeEx
| Like cli.before but also includes argument source (argv, file, env, ...)
...
version.
| ! | For options and operands, final option, all following arguments will be
operands.
|===
...
| Add action to run immediately after each value is parsed, any number of check
actions can be added.
...
| opt.initConfig
| Called internally when an Opt<T> or OptVec<T> is created. The unspecialized
version does nothing, but the function is allowed to call the rest of these
configuration methods. Can be specialized for new types.
...
| cli.newValue
| Used only from transform action callbacks, modifies the current value string
about to be parsed.
...
| opt.defaultPrompt
| Get default name to use in prompts when the msg argument passed to
opt.prompt() is an empty string.
...
| opt.data
| Pointer to value assigned to opt, first value for vectors. Enables use of
std::data(opt).

| opt.empty
| Returns true if opt.size() is 0. Enables use of std::empty(opt).
...
| Action taken after the currently selected command is run, any number can be
added.

| cli.<<guide.adoc#exec-actions, beforeExec>>
| Action taken immediately before the currently selected command is run, any
number can be added.
...
| Default friendly name for type, such as NUM, FLOAT, or STRING. Can be
specialized for new types.
|===
...
| cli.toArgv(out, argc, argv)
| Copy array of wchar_t* to vector of strings. Uses cvt.toString() for
conversions. On conversion error calls cli.badUsage() and returns false.

| cli.toArgvL(out, arg0, ...)
| Copy arguments to vector of strings. Uses cvt.toString(), on error calls
cli.badUsage() and returns false.

| Cli::toArgv(argc, argv)
| Copy array of pointers to vector of string arguments (args).

| Cli::toArgvL(arg0, ...)
| Copy arguments to vector of strings. Uses cvt.toString() to convert
arguments.

| Cli::toPtrArgv(args)
| Create vector of pointers suitable for use with argc/argv APIs, has a
trailing nullptr that is not included in size(). The return values point into
the source vector of strings and are only valid until that vector is resized or
destroyed.
...
Windows). Uses cvt.toString() for conversions. Reports conversion errors via
cli.badUsage() and returns false when they occur.

| cli.toCmdlineL(out,&nbsp;arg0,&nbsp;...)
| Join discrete arguments into a single command line. Uses cvt.toString for
conversions. On conversion error calls cli.badUsage() and returns false.

| Cli::toCmdline(argc,&nbsp;argv) +
Cli::toCmdline(args)
| Join arguments into a single command line. Uses the default conventions (Gnu
or Windows).
...
| cvt.getloc
| Get locale used for conversion to/from string if done via iostream.

| cvt.fromString<T>
| Parses string into any supported type. Can be specialized for new types.
...
| cvt.toString<T>
| Converts value of any supported type into a string. Can be specialized for
new types.
|===
